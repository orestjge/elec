...
enum ResLayout {
  ...
  /// dat の見出しをそのまま出す、昔ながらの組み方。**絵を使わない**——レス番号・
  /// 名前・日時・ID の文字列を 1 行に並べる。
  ///
  /// 他の 2 つは「読むのに要らないものを削る」方向（番号を省く、日付を相対に
  /// する、ID を絵にする）に振ってあるが、それで困る読み方もある。番号を見て
  /// `>>N` を手で書く、ID の文字列をそのまま必死チェッカーへ貼る、書き込みの
  /// 秒数まで見る——長く掲示板を読んできた人ほど、この形が速い。
  classic,
}
